 ...
 *   Single-tenant (MULTI_TENANT = "false"): Bearer token compared against API_TOKEN env var.
 *   Multi-tenant  (MULTI_TENANT = "true"):  JWT verified with JWT_SECRET; `sub` becomes userId.
 *
...
export const app = new Hono<HonoCtx>()
	...
	.use('*', async (c, next) => {
		...
		if ((c.env.MULTI_TENANT as string) === 'true') {
			// Multi-tenant: verify JWT signed with JWT_SECRET; sub becomes userId.
